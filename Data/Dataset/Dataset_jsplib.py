...
class Dataset:
    def __init__(self, filename):
        ...
        self.machine_data = []
        self.pt_data = []
        # data = pd.read_csv(file_path, sep=" ", engine='python', encoding="cp949", skiprows=[0,1,2,3,4], header=None)
        data = pd.read_csv(file_path,
                         sep=r'\s+',  # 하나 이상의 공백을 구분자로 인식
                         encoding="cp949",
                         skiprows=[0, 1, 2, 3, 4],
                         header=None,  # 헤더가 없을 경우
                         engine='python')  # 정규식 사용 시 python 엔진 필요
        ...
        for i in range(self.n_job):
            ...
            for j in range(self.n_machine):
                ...
                # data.iloc[i, 2*j]
                ###
                self.op_data[i].append((data.iloc[i, 2*j] , data.iloc[i, 2*j+1] ))
                self.machine_data[i].append(data.iloc[i, 2*j])
                self.pt_data[i].append(data.iloc[i, 2*j+1])
...
if __name__ == "__main__":
    d = Dataset('JSPLIB/Instances/abz5')
    print()
